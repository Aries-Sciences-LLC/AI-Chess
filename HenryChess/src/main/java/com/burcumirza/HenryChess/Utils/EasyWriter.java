 ...
 *          ISBN 0-9654853-1-5)
 *
 *          EasyWriter provides simple methods for opening and writing to text
 *          files. All exceptions are handled inside the class and are hidden
 *          from the user.
 *
 *
 ...
 *          <xmp> Example: =======
 *
 *          EasyWriter outFile = new EasyWriter("anyname.txt"); if
 *          (outFile.bad()) { System.err.println("Can't create anyname.txt\n");
 *          System.exit(1); } outFile.print("2 + 2 = "); outFile.println(4);
 *          outFile.println(); // an extra blank line outFile.close(); //
 *          optional </xmp>
 *
 */
...
public class EasyWriter {
    ...
    public EasyWriter(String fileName, String mode) {
        ...
        try {
            myOutFile = new PrintWriter(new FileWriter(fileName, "app".equals(mode)));
        } catch (IOException e) {
     ...
     *
     * @return true if en error occurred opening or writing to the file, false
     *         otherwise
     */
    public boolean bad() {
