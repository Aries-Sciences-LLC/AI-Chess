...
public abstract class Piece {
    ...
    public Piece(Chessboard aBoard, int row, int col, int color, int piece, int value, String name) {
        board = aBoard;
        myRow = row;
        myCol = col;
        myColor = color;
    ...
    // abstract method
    public abstract void findMoves(ArrayList<Move> theMoves, boolean actualMove);
    ...
    /*
     * public void setHasMoved(boolean myHasMoved){ hasMoved = myHasMoved; }
     */
    ...
    public void moveDiagonally(ArrayList<Move> moves, boolean actualMove) {
        int toRow, toCol;
        for (int i = 0; i < 2; i++) {
            for (int j = 0; j < 2; j++) {
                toRow = myRow + (1 - 2 * i);
    ...
    public void moveRectilinearly(ArrayList<Move> moves, boolean actualMove) {
        int toRow, toCol;
        for (int i = 0; i < 2; i++) {
            toRow = myRow + (1 - 2 * i);
            toCol = myCol;
