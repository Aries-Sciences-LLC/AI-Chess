...
import com.burcumirza.HenryChess.Utils.*;
...
public class ChessGame {
    ...
    private String levelArray[] = { "Tadpawn", "Knight of the Square Table", "The Bishop Danced", "New Castle",
            "Queen Bee", "King of Pop" };
    private int numLevels = levelArray.length;

    public ChessGame(Chessboard board, int level) {
        this.board = board;
        reverseLevel = numLevels - level;
        playerColor = 0;
        legalMoves = new LegalMoves(board);

        // The "reverseLevel" is used so that the highest level is 0 independent of the
        // number of levels.

        /*
         * if (playerColor==-1){ // Here, the computer plays itself. The test player
         * will start out as black. player[0] = new ComputerPlayer(0, board, applet,
         * legalMoves, 0, numLevels, false); player[1] = new ComputerPlayer(1, board,
         * applet, legalMoves, 0, numLevels, true); }
         */
        if (level == -1) {
    ...
    public void makeMove() {
        if (move >= 0) {
            ...
            if (piece.piece() == 3 || piece.piece() == 5) {
                piece.hasMoved = true;
            }
    ...
    public void reSet() {
        ...
        player[playerColor] = new HumanPlayer(playerColor, board, this);
        player[(playerColor + 1) % 2] = new ComputerPlayer((playerColor + 1) % 2, board, this, legalMoves, reverseLevel,
                numLevels, false);
        board.update();
        movesSinceCapture = -1;
        move = -1;
