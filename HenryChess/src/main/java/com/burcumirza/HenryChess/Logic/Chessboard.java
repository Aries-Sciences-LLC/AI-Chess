...
public class Chessboard extends JPanel {
    ...
    private Piece[] knight = new Piece[4];
    private Piece[] bishop = new Piece[4];
    private Piece[] pawn = new Piece[16];
    ...
    private ArrayList<BoardPosition> positionList = new ArrayList<BoardPosition>();
    ...
    public void resetBoard() {
        ...
        for (int color = 0; color < 2; color++) {
            for (int i = 0; i < 8; i++) {
                pawn[i * (color + 1)] = new Pawn(this, i, 1 + 5 * color, color);
            }
            for (int i = 0; i < 2; i++) {
                castle[i + 2 * color] = new Castle(this, i * 7, 7 * color, color);
                knight[i + 2 * color] = new Knight(this, 1 + i * 5, 7 * color, color);
                bishop[i + 2 * color] = new Bishop(this, 2 + i * 3, 7 * color, color);
            }
            queen[color] = new Queen(this, 3, 7 * color, color);
    ...
    public boolean checkIfPiece(int pieceValue, int color) {
        Iterator<Piece> iter = pieceList(color).iterator();
    ...
    public List<Piece> pieceList(int color) {
    ...
    public void paintComponent(Graphics g) {
        ...
        ((Graphics2D) g).setPaint(new GradientPaint(400, 0, new Color(55, 98, 103), 400, 700, new Color(40, 41, 61)));
        ...
        for (int row = 0; row < 8; row++) {
            for (int col = 0; col < 8; col++) {
                ...
                } else if (flash && displayCount != 0 && displayCount % 2 == 0 && row == (7 - newCol)
                        && col == newRow) {
                    g.setColor(RED);
                    g.fillRect(x, y, CELLSIZE, CELLSIZE);
                } else if ((row + col) % 2 == 1) {
