...
public class Move {
    static public Chessboard board;
    ...
    private boolean queened = false, opening = false, gameOver = false;
    ...
    public boolean equals(Move aMove) {
        return aMove.piece() == this.piece() && aMove.newRow() == this.newRow() && aMove.newCol() == this.newCol();
    }
    ...
    public String getOpeningMoveString() {
        ...
        moveString += "openingMove[" + move + "]" + " = new Move(board.objectAt (" + oldRow + "," + oldCol + "),"
                + newRow + "," + newCol + ", true);\n";
        if (castleIndex() != -1) {
            moveString += "openingMove [" + move + "].changeCastleIndex(1);\n";
        } else if (capturedPiece != null) {
            moveString += "openingMove [" + move + "].addCapturedPiece(board.objectAt(" + newRow + "," + newCol
                    + "));\n";
        }
        return moveString;
    }
    ...
    public String toString() {
        /*
         * return piece.name() + " moves from " + rowArray[oldRow] + (oldCol+1)+ " to "
