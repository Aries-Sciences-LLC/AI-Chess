...
public class Strategy{
    ...
    public Strategy(Chessboard board, LegalMoves legalMoves, int reverseLevel, int numLevels, boolean testPlayer){	
        ...
        System.out.println("ReverseLevel = " + reverseLevel);
   ...
   public Move chooseMove(ArrayList moveList){
        System.out.println("Movelist size at 29 = " + moveList.size());
        Move move = null;
        int color = ((Move)moveList.get(0)).piece().color();
        if (opening) checkOpening(color);
        else if (!endGame) checkEndgame();
        int i=0;
        if (reverseLevel>1){
        ...
        System.out.println("Movelist size at 44 = " + moveList.size());
        moveList = evaluate(moveList,color);
        System.out.println("Movelist size at 46 = " + moveList.size());
        if (moveList.size()>1){
            ...
            System.out.println("Movelist size at 61 = " + moveList.size());
        }
        return move = (Move)moveList.get(rGen.nextInt(moveList.size()));
    }
    
     public ArrayList evaluate(ArrayList moveList, int color){
        Move aMove;
        //"playNext" && "oppNext" include defence of own pieces
        List playMoves, oppMoves, playNext, oppNext;
        ...
        for (int i=0; i<moveList.size(); i++){
            ...
            if (oppMoves.size()==0){
                ...
                oppNext = legalMoves.getMoveList((color+1)%2,false);
        ...
        for (int n=0; n<moveList.size(); n++){
            Move thisMove = (Move)moveList.get(n);
            System.out.println(thisMove.toString() + " value " + thisMove.value());
        }

        return bestMoves(moveList);    
    }
    
     public ArrayList evalDevelopement (ArrayList moveList, int color){
         Move aMove;
         for (int i=0; i<moveList.size(); i++){
             aMove = (Move)moveList.get(i);  
             if (opening){
             ...
             else if (board.movesSinceCapture()>45 && aMove.capturedPiece()!=null
                    && aMove.value()>0 
 ...
 public int evalEndGame(Move aMove,int aColor)  {
       ...
       int color = aColor;
       int oldRow = aMove.oldRow(); int oldCol = aMove.oldCol(); 
       int newRow = aMove.newRow(); int newCol = aMove.newCol();
       switch (piece.piece()){
            case 0:		//pawn
           ...
           case 1:              //knights
               value+=
               Math.abs(oldRow-board.king[(color+1)%2].myRow)
               + Math.abs(oldCol-board.king[(color+1)%2].myCol)
                   - (Math.abs(newRow-board.king[(color+1)%2].myRow)
               + Math.abs(newCol-board.king[(color+1)%2].myCol));
               break;
           case 2:              //bishops
           ...
           case 4:              //queen
            ...
            case 5:          // king
               ...
               if (newRow==0 && (newCol==0||newCol==7) 
               || newRow==7 && (newCol==0||newCol==7)){
                   value-=2;
               }
    ...
    public int evalMiddle(Move aMove, int aColor){
       ...
       switch (piece.piece()){
          ...
          case 3:              //castle
               ...
               else if (newCol == 6-5*color && board.king[(color+1)%2].myCol==7*color
                       && Math.abs(board.king[(color+1)%2].myRow)-newRow<4){
                   value+=1;
               }
   ...
   public int evalOpening(Move aMove, int aColor){
       ...
       if (aMove.castleIndex()!=-1){
           ...
           switch (piece.piece()){
                ...
                case 2:		//bishop
                    if (oldCol==7*color){
                        if (newCol == 4-color || 
                                newCol == 1+5*color && (newRow == 1 || newRow == 6)) value+=3;
    ...
    public int evalRepeat(Move aMove){
        ...
        if (lastMove.piece().equals(aMove.piece())){
            value-=1;
            if (lastMove.oldCol()==aMove.newCol()
                 && lastMove.oldRow()==aMove.newRow()){
                value-=1;
            }
        }
        return value;
...
/* Returns evalPieces (playercolor) - evalPieces(oppColor)
 * Seems that this could be done in another method.
 */    
    public int pieceDifference(int color){
        return evalPieces(color)-evalPieces((color+1)%2);
    }
    
 /* This method added to pieceDifference takes into account the potential trade of pieces.
  * One thing that is tweakable is the value of pieces. For example, are two bishops as
  * valuable as two knights? What's the value of a king?
  */    
    public int evalDefence(int color, List playNext,List oppNext){
        int valueAdded = 0;
        Move aMove;
        Iterator iter = board.pieceList(color).iterator();
        while (iter.hasNext()){
            Piece piece = (Piece)iter.next();
            ArrayList attackerList = new ArrayList();
            ...
            if (attackerList.size()>0){
                ArrayList defenderList = new ArrayList();
                ...
                Iterator iterA = insertionSort(attackerList).iterator();
                Iterator iterD = insertionSort(defenderList).iterator();
...
// Returns true if none of players own pawns are in the row.
    public boolean evalPawnInFile (int color, int row, int ignoreCol){    
        ...
        while (col<7 && pawnInFile == false){
            if (col!=ignoreCol && board.objectAt(row,col)!=null 
                    && board.objectAt(row,col).color()==color
 ...
 // returns 1,0 or -1 depending on whether it is moving toward, -- , or away from center
   ...
   // returns the number of moves a piece has
    ...
    public int evalPieceMoves(Piece piece, List nextMoves){
        int pieceMoves = 0;
    ...
    public int evalPieces(int color){
        ...
        Iterator iter = board.pieceList(color).iterator();
        while (iter.hasNext()){
            valueAdded+=((Piece)iter.next()).value();
        }	
        return valueAdded;
    }


/* Determines whether it's still the opening checking whether the player has castled 
 & the  and bishops
 * knigh
 
 * This can definitely be improved. Returns boolean "false" if it determines that 
 * it is no longer the opening.*/
...
/* Determines whether it's end game by counting the value of the pieces on the board. 
 * This can definitely be improved. Returns boolean "true" if it determines that 
 * it is the endgame.*/
    
    public void checkEndgame(){
       if (evalPieces(0)+ evalPieces(1)< valAtEndGame){
           endGame = true;
           System.out.println("endgame endgame endgame endgame endgame endgame endgame");
       }
   }
...
/* This finds the extreme value of the moveList (either the largest or smallest).
 * If max = true, it finds the max, of course.  
 */  
    public int findExtreme(List moveList, boolean max){
        int extreme = ((Move)moveList.get(0)).value();
        for (int i=1; i<moveList.size(); i++){
            if (max && ((Move)moveList.get(i)).value()>extreme
            || !max && ((Move)moveList.get(i)).value()<extreme){
                    extreme = ((Move)moveList.get(i)).value();
            }
        }
        return extreme;
    }
   
/*Eliminates from moveList moves that aren't valued the greatest within the given width
Is sent the max. */
    
    public ArrayList bestMoves(ArrayList moveList){
        int i = 0; Move aMove;
       ...
       public void traverseTree(TreeNode node){
           ...
           if (node.level()!=0){
                ...
                ((Move)node.value()).setValue(extreme);
           }
	}
        
    public void buildTree(TreeNode node, ArrayList moveList, int level){
        Move aMove = (Move)moveList.get(0);
